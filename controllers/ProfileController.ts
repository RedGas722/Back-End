...
let profile = async (req: Request, res: Response) => {
  try {    
    ...
    return res.status(200).json(
      { status: 'Get profile Ok', id: idUser, name: nameUser, cc:ccUser, email: emailUser, telefono: telefonoUser, direccion: direccionUser, imagen: imagenUser }
    );
  } catch (error: any) {
